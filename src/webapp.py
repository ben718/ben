    ...
    <style>
      ...
      * {
      ...
      .stepper li.active {
        ...
        box-shadow: 0 16px 36px -28px rgba(37, 99, 235, 0.6);
  ...
  <body>
    <main>
      ...
      <div class=\"layout\">
        <div class=\"flow\">
          ...
          <ol class=\"stepper\" id=\"stepper\">
            <li data-step=\"1\" class=\"active\" role=\"button\" tabindex=\"0\">1. Environnement<small>Surface, structure, interférences</small></li>
            <li data-step=\"2\" role=\"button\" tabindex=\"0\">2. Services<small>Usages, sécurité, bande passante</small></li>
            <li data-step=\"3\" role=\"button\" tabindex=\"0\">3. Vidéosurveillance<small>Caméras, stockage, modes</small></li>
            <li data-step=\"4\" role=\"button\" tabindex=\"0\">4. Dossier<small>Livrables et export</small></li>
          </ol>
      ...
      <section class=\"step\" data-step=\"4\">
        ...
        <p class=\"description\">Consultez la synthèse en direct dans la colonne de droite, ajustez vos réponses si nécessaire puis exportez le dossier complet.</p>
        <div class=\"callout\">
          <strong>Affinez avant export</strong><br />
          Chaque modification sur les étapes précédentes met instantanément à jour la recommandation matérielle, les indicateurs PoE et les alertes de services. Avancez/reculer dans le parcours pour vérifier les impacts puis lancez la génération.
        </div>
      ...
      <nav class=\"actions\">
        ...
        <aside class=\"sidebar\" aria-label=\"Synthèse en temps réel\">
          <div class=\"sidebar-inner\">
            <div class=\"sidebar-heading\">
              <strong>Synthèse en temps réel</strong>
              <span>Analyse en continu de la configuration OmadaBOM.</span>
            </div>
            ...
            <p style=\"margin:0;font-size:0.92rem;color:rgba(15,23,42,0.7);\">Les éléments listés ici reflètent immédiatement vos réponses aux questionnaires simples et avancés.</p>
          </div>
        </aside>
      </div>
    ...
    <script>
      ...
      stepperItems.forEach((item, index) => {
        const stepIndex = index + 1;
        item.addEventListener('click', () => showStep(stepIndex));
...
def export_static_site(
    ...
    *,
    ...
    """Write the homepage template to *destination* and return the path.

    The exported document mirrors what the embedded WSGI server would render.
    This is primarily used to publish a static preview (e.g. via GitHub Pages)
    where the Python backend n'est pas disponible.  When the user interagit
    avec l'interface sur cette version statique, les appels réseau échoueront,
    mais l'interface informera déjà l'utilisateur de l'absence du moteur.
    """
    ...
    html = _render_homepage(info_provider(), history_provider())
...
def serve(
    ...
    *,
    ...
    export_static: str | Path | None = None,
) -> None:
    ...
    export_static:
        Optional path to a static HTML file to generate instead of launching the
        server.  This permet de publier rapidement une démo sur GitHub Pages.
    """

    if export_static is not None:
        export_static_site(Path(export_static))
        return
...
def _parse_args(
    ...
    parser.add_argument(
        "--export-static",
        dest="export_static",
        help="Écrit la page HTML statique dans le chemin fourni et quitte",
    )
    args = parser.parse_args(argv)
    return args.host, args.port, args.catalogue, args.export_static


def main(argv: list[str] | None = None) -> None:
    host, port, catalogue, export_static_path = _parse_args(argv)
    serve(host, port, catalogue=catalogue, export_static=export_static_path)
